...
bool Expr::TraverseDataDependency(DataDepVisitor *visitor, Env *env)
{
  switch ( expr_type_ )
  {
    case EXPR_ID:
          ...
          case UNION_VAR:
            {
              Field* field = env->GetField(id());
              if ( field ) {
                field->Traverse(visitor, env);
              } else {
                DEBUG_MSG("Id %s is not a field\n", id()->Name());
              }
            }
            break;
          case MACRO:
    ...
    case EXPR_MEMBER:
      /*
        ...
        not defined in the current environment.)
      */
      if ( ! operand_[0]->Traverse(visitor, env) )
        return false;
      break;
    ...
    default:
      ...
      {
        if ( operand_[i] && 
             ! operand_[i]->Traverse(visitor, env) )
        {
          return false;
        }
      }
...
bool CaseExpr::TraverseDataDependency(DataDepVisitor *visitor, Env *env)
{
  foreach(i, ExprList, index_)
    if ( ! (*i)->Traverse(visitor, env) )
      return false;
  return value_->Traverse(visitor, env);
}
